...
import static tv.mta.flutter_playout.video.ConstantsKt.AES_ALGORITHM;
import static tv.mta.flutter_playout.video.ConstantsKt.AES_TRANSFORMATION;
...
public class PlayerLayout extends PlayerView implements FlutterAVPlayer, EventChannel.StreamHandler {
    ...
    public PlayerLayout(@NonNull Context context,
        ...
        try {
            ...
            this.aesKey = args.getString("aesKey");
...
//        mPlayerView.addAnalyticsListener(new PlayerAnalyticsEventsListener());
    ...
    private void updateMediaSource() {
        ...
        // videoSource = new
        // ProgressiveMediaSource.Factory(dataSourceFactory).createMediaSource(Uri.parse(this.url));
        ...
        Log.d("#dada", "aesKey = " + this.aesKey);
        Log.d("#dada", "encryptedFile = " + encryptedFile.getPath());
        Log.d("#dada", "uri = " + uri.toString());

        byte[] keyBytes = hexStringToByteArray(this.aesKey);
...
//                new SecretKeySpec(keyBytes, AES_ALGORITHM),
    ...
    private void updateMediaSource2() {
        /* Produces DataSource instances through which media data is loaded. */
        DataSource.Factory dataSourceFactory = new DefaultDataSourceFactory(context,
                Util.getUserAgent(context, "flutter_playout"));
    ...
    /**
     * set text track language for player - language must be one of available in HLS
     * manifest
